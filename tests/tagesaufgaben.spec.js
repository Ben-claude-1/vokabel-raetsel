...
// Lernminuten des Tages fälschen. `min` = {grammatik, en, es} in Minuten.
async function tagesstand(page, min) {
  const m = min || {};
  await page.route('**/rest/v1/learn_sessions*', async (route) => {
    if (route.request().method() !== 'GET') return route.fulfill({ status: 200, contentType: 'application/json', body: '[]' });
    const now = new Date().toISOString();
    const sessions = [];
    const add = (n, row) => { for (let i = 0; i < (n || 0); i++) sessions.push(Object.assign({ active_seconds: 60, started_at: now }, row)); };
...
async function startseite(page, min) {
  await blockWrites(page);
  await tagesstand(page, min);
