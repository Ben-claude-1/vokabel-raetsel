...
// `gutschrift` = {grammatik, en, es} Gutschriftpunkte (40 = eine Minute).
async function tagesstand(page, min, extra) {
  ...
  await page.route('**/rest/v1/learn_sessions*', async (route) => {
    ...
    const sessions = [];
    const add = (n, row) => { for (let i = 0; i < (n || 0); i++) sessions.push(Object.assign({ active_seconds: 60, started_at: now, correct_count: 0, wrong_count: 0, skipped_count: 0, credit_points: 0 }, row)); };
    ...
    const r = x.gutschrift || {};
    const gut = (lang, game, punkte) => sessions.push({ game, language: lang, active_seconds: 0, started_at: now,
      correct_count: Math.round(punkte / 5), wrong_count: 0, skipped_count: 0, credit_points: punkte });
    if (r.grammatik) gut('en', 'grammatik', r.grammatik);
    if (r.en) gut('en', 'leiterspiel', r.en);
    if (r.es) gut('es', 'leiterspiel', r.es);
    ...
    if (x.falsch || x.uebersprungen) sessions.push({ game: 'leiterspiel', language: 'en', active_seconds: 0, started_at: now,
      correct_count: 0, wrong_count: (x.falsch || 0) + (x.uebersprungen || 0), skipped_count: x.uebersprungen || 0, credit_points: 0 });
...
test.describe('Tagesaufgaben', () => {
  ...
  test('Gutschrift für richtige Antworten verkürzt die Zeit', async ({ page }) => {
    // 80 Gutschriftpunkte auf Englisch (16 Antworten in Topf 4) → Ziel 8 statt
    // 10 Minuten; mit 8 Minuten Lernzeit ist die Aufgabe damit erledigt.
    await startseite(page, { en: 8 }, { gutschrift: { en: 80 } });
    ...
    await expect(page.locator('text=Schwere Vokabeln zählen mehr')).toBeVisible();
    await expect(page.locator('text=1 von 3 geschafft')).toBeVisible();
  });
